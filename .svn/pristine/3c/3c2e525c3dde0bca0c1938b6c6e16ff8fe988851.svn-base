...
class WxValidate {
	...
	__initMethods() {
		...
		that.methods = {
			 ...
			 */
			tel(value) {
        return that.optional(value) || /(^(13\d|14[57]|15[^4,\D]|17[135678]|18\d)\d{8}|170[^346,\D]\d{7})$/.test(value)
			},
      /**
			 * 验证中文
			 */
      nameChina(value) {
        return that.optional(value) || /^[\u4E00 - \u9FA5]{2,}$/.test(value)
        },
        /**
          * 验证车架号
          */
      vin(value) {
        return that.optional(value) || /^[a-zA-Z0-9]{17}$/.test(value)
      },
			/**
			 * 验证URL格式
			 */
